...
- (NSMutableArray *)searchModWithFilters:(NSDictionary<NSString *, NSString *> *)searchFilters previousPageResult:(NSMutableArray *)modrinthSearchResult {
    int limit = 50;
    NSMutableString *facetString = [NSMutableString stringWithFormat:@"[[\"project_type:%@\"]", ([searchFilters[@"isModpack"] boolValue] ? @"modpack" : @"mod")];
...
- (void)loadDetailsOfMod:(NSMutableDictionary *)item {
    ...
    if (!response) return;
    NSMutableArray *names = [NSMutableArray arrayWithCapacity:response.count];
...
- (void)downloader:(MinecraftResourceDownloadTask *)downloader submitDownloadTasksFromPackage:(NSString *)packagePath toPath:(NSString *)destPath {
    ...
    NSDictionary *depInfo = [ModpackUtils infoForDependencies:indexDict[@"dependencies"]];
    if (depInfo[@"json"]) {
        NSString *jsonPath = [NSString stringWithFormat:@"%s/versions/%@/%@.json", getenv("POJAV_GAME_DIR"), depInfo[@"id"], depInfo[@"id"]];
    ...
    PLProfiles.current.profiles[indexDict[@"name"]] = [@{
        ...
        @"icon": [NSString stringWithFormat:@"data:image/png;base64,%@", [[NSData dataWithContentsOfFile:tmpIconPath] base64EncodedStringWithOptions:0]]
    } mutableCopy];
