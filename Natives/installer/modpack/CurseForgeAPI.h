...
// Pending properties to hold the manifest and file paths until the user presses "Play."
...
@property (nonatomic, strong) NSDictionary *pendingModpackDetail; // Original modpack detail (if needed)
...
- (void)installModpackFromDetail:(NSDictionary *)detail atIndex:(NSInteger)index {
    ...
    // Immediately process the modpack manifest and create the profile.
    [self processManifestFromPackage:zipURL destinationPath:[NSString stringWithFormat:@"%s/custom_gamedir/%@", getenv("POJAV_GAME_DIR"), detail[@"id"]] downloader:nil];
}

/// New method: Process the modpack manifest (download, extract, parse, create profile),
/// but do NOT start the subfile downloads. Instead, store pending info and notify UI.
- (void)processManifestFromPackage:(NSURL *)zipURL destinationPath:(NSString *)destPath downloader:(MinecraftResourceDownloadTask *)downloader {
    // Assume zipURL points to the modpack zip file that is already downloaded locally.
    // In a real app, you might need to download the file first.
    // Here we use zipURL.path as the local file path.
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        // Process profile creation on the main thread.
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            // Now store pending info for subfile downloads.
            self.pendingManifest = manifest;
            self.pendingPackagePath = packagePath;
            self.pendingDestinationPath = destPath;
            
            // Notify the UI that the modpack is ready for play (i.e. heavy file downloads can be started when the user taps "Play").
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ModpackReadyForPlay" object:self];
        });
    });
}

/// New method: Start downloading individual mod files using the stored manifest.
- (void)startPendingDownload {
    ...
    // Process file downloads on a background queue.
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (NSDictionary *cfFile in filesArr) {
            ...
            if (!downloadUrl && required) {
                ...
                // Optionally, fallback to browser if a required file is missing.
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self fallbackOpenBrowserWithURL:self.fallbackZipUrl];
                });
                return;
            ...
            // Assume you have a downloader instance to create a download task.
            // Here we post a notification so that your downloader process can handle the file download.
...
#pragma mark - Downloader for Subfiles (Existing code for download tasks)
