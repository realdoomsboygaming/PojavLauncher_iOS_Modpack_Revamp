...
#import "utils.h"
#include <dlfcn.h>  // :contentReference[oaicite:0]{index=0}
...
- (void)viewDidLoad {
    ...
    // Dynamically load WorkflowUIServices and create progress view
...
- (void)tableViewDidSelectSection:(UITapGestureRecognizer *)sender {
    UITableViewHeaderFooterView *view = (UITableViewHeaderFooterView *)sender.view;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSURLSessionDownloadTask *downloadTask = [self.afManager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull progress) {
       ...
       dispatch_async(dispatch_get_main_queue(), ^{
           ...
           LauncherNavigationController *navVC = (LauncherNavigationController *)((UISplitViewController *)self.presentingViewController).viewControllers[1];
